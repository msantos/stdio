defmodule StdioRootlessTest do
  use ExUnit.Case

  @moduletag :linux
  doctest Stdio.Rootless
  ...
  test "sh: exec process writing to stdout" do
    result = Stdio.stream!("echo test", Stdio.Rootless) |> Enum.take(1)
    assert [stdout: "test\n"] = result
  end
  ...
  test "pipe: subprocess exits" do
    result =
      ["x", "x", "x", "x"]
      |> Stdio.pipe!("exit 3", Stdio.Rootless)
      |> Enum.to_list()
  ...
  test "mount: verify mount flags" do
    [{:stdout, e} | _] = Stdio.stream!("mount", Stdio.Rootless) |> Enum.to_list()
  ...
  test "mount: /opt from relative path" do
    result =
      Stdio.stream!("PATH=/opt:$PATH stdiotrue", Stdio.Rootless, fstab: ["opt"])
      |> Enum.to_list()
